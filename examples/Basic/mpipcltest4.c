 ...
 * This program tests MPIA_Startall, MPIA_Pready, and MPIA_Waitall functions.
...
int main(int argc, char* argv[])
{
    ...
    { /* sender */
        ...
        {
            rc = MPIA_Psend_init(buf, nparts, count, MPI_DOUBLE, i, tag,
                                 MPI_COMM_WORLD, MPI_INFO_NULL, &req[i - 1]);
            assert(rc == MPI_SUCCESS);
        }

        rc = MPIA_Startall(NNEIGHBORS, req);
        assert(rc == MPI_SUCCESS);
...
#pragma omp parallel for private(j) shared(buf, req) num_threads(nparts)
        for (i = 0; i < nparts; i++)
        {
            ...
            {
                rc = MPIA_Pready(i, &req[j]);
                assert(rc == MPI_SUCCESS);
            }
        }

        rc = MPIA_Waitall(NNEIGHBORS, req, MPI_STATUSES_IGNORE);
        assert(rc == MPI_SUCCESS);
    ...
    { /* receiver */
        ...
        rc = MPIA_Precv_init(buf, nparts, count, MPI_DOUBLE, 0, tag,
...
#pragma omp critical
                ...
                {
                    /* do some other work */
                    rc = MPIA_Test(&req, &testflag, MPI_STATUS_IGNORE);
                    assert(rc == MPI_SUCCESS);
                    /* do some other work based on testflag */
                }
            }
        }
        rc = MPIA_Wait(&req, MPI_STATUS_IGNORE);
        assert(rc == MPI_SUCCESS);
