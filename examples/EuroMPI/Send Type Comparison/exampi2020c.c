...
int main(){
  ...
  if(rank == 0){
	...
	for(buffer_size = 1; buffer_size <= MAX_BUFF_SIZE ; buffer_size<<=1){
      ...
      start = MPI_Wtime();
      MPI_Send_init(buffer, buffer_size, MPI_INT, 1, 0, MPI_COMM_WORLD, request);
  ...
  if(rank == 1){
	  for(buffer_size = 1; buffer_size <= MAX_BUFF_SIZE ; buffer_size<<=1){
		...
		MPI_Recv_init(buffer, buffer_size, MPI_INT, 0, 0, MPI_COMM_WORLD, request);
		for(int k = 0; k<RUNS; k++){			
			MPI_Start(request);
			MPI_Wait(request, MPI_STATUS_IGNORE);
		}
