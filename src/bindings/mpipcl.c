...
#ifdef __cplusplus
    MPIPCL_REQUEST* request = new MPIPCL_REQUEST;
    *wrapper = (uintptr_t)request;
#else
    MPIPCL_REQUEST* request = (MPIPCL_REQUEST*)malloc(sizeof(MPIPCL_REQUEST));
    *wrapper = (uintptr_t)request;
#endif
...
int MPIPCL(_Pready)(int partition, MPIA_REQUEST* wrapper)
{
    MPIPCL_DEBUG("INSIDE PREADY\n");
    // check for calling conditions
    MPIPCL_REQUEST* request = (MPIPCL_REQUEST*)*wrapper;
                          ...
                          MPIA_REQUEST* wrapper)
{
    ...
    {
        int ret_val = MPIPCL(_Pready)(i, wrapper);
...
// Other Wait function calls basic test, modifying MPIX_Wait will propagate the
// changes.
                     ...
                     MPI_Status* status)
{
    ...
    while (!flag)
    {
        ...
        { /* NOTE: MPI_Status object is not updated */
            int ret_val =
                MPIPCL(_Test)(&array_of_requests[i], &flag, MPI_STATUS_IGNORE);
            assert(MPI_SUCCESS == ret_val);
            if (flag == 1)
            {
                *index = i;
...
// Other test function calls basic test, modifying MPIX_Test will propagate the
// changes.
int MPIPCL(_Test)(MPIA_REQUEST* wrapper, int* flag, MPI_Status* status)
{
    *flag                   = 0;
    ...
    {
        MPIPCL_DEBUG(
            "Early MPIX Test exit due to null or inactive request: %p\n",
            (void*)request);
        return MPI_SUCCESS;
    }
                     ...
                     MPI_Status array_of_statuses[])
{
    ...
    *flag = 1;
    ...
    {
        int ret_val =
            MPIPCL(_Test)(&array_of_requests[i], &myflag, MPI_STATUS_IGNORE);
        assert(MPI_SUCCESS == ret_val);
        *flag = *flag & myflag;
    }
...
{
    MPIPCL(_Request_free)((MPIPCL_REQUEST*)*wrapper);
}
